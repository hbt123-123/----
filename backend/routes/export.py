...
def preview_summary(pid):
    ...
    materials, rows = _build_summary(dir_)
    return jsonify(materials=materials, rows=rows, project_name=meta.get('name', ''))
...
def download_summary(pid):
    ...
    for row in rows:
        ws.append([row['no'], row['name'], row['leader'], row['student_id'],
                  ...
                  + row['statuses'] + [row['remark']])

    # 列宽
    widths = [6, 16, 10, 12, 13, 22, 10] + [14] * len(materials) + [26]
